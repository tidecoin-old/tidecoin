...
- [Development guidelines](#development-guidelines)
    - [General Tidecoin Core](#general-tidecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Tidecoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play tidecoins" on the test network, if you
...
Tidecoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/tidecoin-project/tidecoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_tidecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_tidecoin --log_level=test_suite
$ valgrind -v --leak-check=full src/tidecoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_tidecoin.coverage/index.html`.
```
...
Tidecoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Tidecoin Core, where everyone uses
...
pay attention to for reviewers of Tidecoin Core code.

General Tidecoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Tidecoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Tidecoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Tidecoin Core so that they can be integrated
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Tidecoin Core, the wallet was always
